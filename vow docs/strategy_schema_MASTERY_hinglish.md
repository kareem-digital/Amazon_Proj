...
### 1. Zero Hallucination — apne mann se kuch nahi

Agent kabhi bhi koi number ya value **khud nahi banayega**. Har cheez ya API se aayegi, ya registry se, ya user ne boli hogi.
...
| A6 | `confirm_understanding` | "Maine ye samjha — sahi hai?" | filled slots | message | ⚠️ Fix chahiye |
...
|---|---|---|
| 1 | `resolve_locations` | David ka comment hai, API confirmed hai, self-contained hai |
| 2 | `derive_channel_tier` | Sab kuch iske upar khada hai (matching, tier, forecast) |
| 3 | `validate_targeting` | Chhota, rules clear hain, David ke comment ka seedha jawab |
| 4 | `compute_effective_cpm` | Bina iske forecast galat number dega |
| 5 | `normalise_deal_currency` | 9% error, aur chhupa hua error hai |

**Uske baad:**
6. `state_uncertainty` — chauthe principle ka enforcement
7. `assess_forecast` + `repair_plan` — bid lever ke saath
8. `build_create_payload` — `automated-strategies` ka shape
9. `resolve_dates`, `ask_missing` — conversation polish
