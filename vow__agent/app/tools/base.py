...
class BaseVOWTool:
    ...
    async def _request(
        ...
        for attempt in range(1, self.settings.vow_api_max_retries + 1):
            ...
            try:
                ...
                if response.status_code == 401:
                    raise VowAuthError(
                        "Authentication failed", status_code=401, endpoint=path
                    )
                if response.status_code == 403:
                    raise VowAuthError(
                        "Forbidden", status_code=403, endpoint=path
                    )
                ...
                elapsed = time.monotonic() - start
                last_error = VowApiError(
                    f"Timeout after {elapsed:.1f}s", endpoint=path
                )
                if attempt < self.settings.vow_api_max_retries:
                    logger.warning(
                ...
                last_error = VowApiError(
                    f"Network error: {exc}", endpoint=path
                )
                if attempt < self.settings.vow_api_max_retries:
                    logger.warning(
        ...
        raise last_error or VowApiError(f"Failed after retries", endpoint=path)
