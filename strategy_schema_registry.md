...
**Document Version:** 1.1.0 (Enhanced with Edge-Case Workflows & Prompt Parsing Rules)  
...
## 7. Advanced Agent Recovery & Edge-Case Architecture

### 7.1 Automated Brief Parsing & Slot Extraction Prompting Rules
When a user provides a free-text strategy brief (e.g. *"I want a Prime Video awareness campaign in the UK for £10,000 running in August for my education website..."*), the agent applies the following extraction heuristic:

1. **Entity Normalization**:
   - `August 2026` $\rightarrow$ `flight_dates: { lower: "2026-08-01", upper: "2026-08-31" }`
   - `UK` $\rightarrow$ `markets: ["GB"]`, `primary_currency: "GBP"`
   - `£10,000` $\rightarrow$ `market_budgets: [{ market: "GB", budget: "10000.00", base_bid: "30.00" }]`
   - `education website` $\rightarrow$ `product_location: "NOT_SOLD_ON_AMAZON"`, `product_categories: [1]`

2. **Validation Failure Protocols**:
   ...
   - **Past Dates**: If `flight_dates.lower` $<$ today, auto-adjust to tomorrow's date and inform user.

3. **Auto-Widening Forecast Repair Loop (Zero Reach Handling)**:
   If `POST /api/strategies/reach-forecast/` returns `estimated_unique_reach == 0` or insufficient frequency:
   - **Action 1**: Switch audience set bundle from `Narrow` to `Balanced` or `Broad`.
   - **Action 2**: Adjust base CPM bid up to market recommended floor (e.g. increase from £15 to £30 for Prime Video).
   - **Action 3**: Re-run forecasting engine to verify non-zero reach projection before rendering final Strategy Card.
